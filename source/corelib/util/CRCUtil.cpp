...
S_ULONG_32 CRCUtil::crc_table32_[] = 
{
...
S_ULONG_32 CRCUtil::build_crc( const S_UINT_8 *buffer, int bytes)
{
    S_ULONG_32 crc = 0xFFFFFFFF;

    for( const S_UINT_8 *p = buffer; bytes > 0; bytes--)
	{
        crc = ( crc >> 8 ) ^ crc_table32_[(S_UINT_8) ((S_UINT_8) crc ^ *p++ ) ];
    }

    return ( crc ^ 0xFFFFFFFF );
}

S_ULONG_32 CRCUtil::append_2_crc( S_ULONG_32 previousCRC, const char *rawData, S_ULONG_32 size)
{
	S_ULONG_32 crc = previousCRC ^ 0xFFFFFFFF;

	for ( const S_UINT_8 *p = (const S_UINT_8 *)rawData; size > 0; size-- ) 
	{
		crc = ( crc >> 8 ) ^ crc_table32_[(S_UINT_8) ((S_UINT_8) crc ^ *p++ ) ];
	}
...
S_ULONG_32 CRCUtil::create_crc( const char *string )
{
    S_ULONG_32 crc = 0xFFFFFFFF;

    for ( const S_UINT_8 *p = (S_UINT_8*)string; *p!=0; ++p) 
    {
        crc = ( crc >> 8 ) ^ crc_table32_[(S_UINT_8) ((S_UINT_8) crc ^ *p ) ];
    }
...
S_ULONG_32 CRCUtil::create_crc( const char *rawData, S_ULONG_32 len)
{
    return build_crc((const S_UINT_8 *)rawData, len);
}

S_ULONG_32 CRCUtil::build_crcnocase( const S_UINT_8 *buffer, int bytes)
{
    S_ULONG_32 crc = 0xFFFFFFFF;

    for ( const S_UINT_8 *p = buffer; bytes > 0; bytes-- ) 
	{
		S_UINT_8 ch = ACE_OS::ace_tolower(*p++);
		//S_UINT_8 ch = tolower(*p++);
        crc = ( crc >> 8 ) ^ crc_table32_[(S_UINT_8) ((S_UINT_8) crc ^ ch ) ];
    }

    return ( crc ^ 0xFFFFFFFF );
}

S_ULONG_32 CRCUtil::create_crcnocase( const char *string)
{
    S_ULONG_32 crc = 0xFFFFFFFF;

    for ( const S_UINT_8 *p = (S_UINT_8*)string; *p!=0 > 0; ++p ) 
    {
        //S_UINT_8 ch = tolower(*p);
		S_UINT_8 ch = ACE_OS::ace_tolower(*p);
        crc = ( crc >> 8 ) ^ crc_table32_[(S_UINT_8) ((S_UINT_8) crc ^ ch ) ];
...
S_ULONG_64 CRCUtil::crc_table64_[] =
{
...
S_ULONG_64 CRCUtil::build_crc64( const S_UINT_8 *buffer, int bytes)
{
    S_ULONG_64 crc = 0;

    for ( const S_UINT_8 *p = buffer; bytes > 0; bytes-- ) 
    {
        crc = crc_table64_[(crc ^ *p++) & 0xff] ^ (crc >> 8);
    }

    return crc;
}

S_ULONG_64 CRCUtil::append_2_crc64( S_ULONG_64 previousCRC, const char *rawData, S_ULONG_32 bytes)
{
    S_ULONG_64 crc = previousCRC;
    const S_UINT_8* buffer = (const S_UINT_8*) rawData;
...
S_ULONG_64 CRCUtil::create_crc64( const char *string )
{
    S_ULONG_64 crc = 0;
...
S_ULONG_64 CRCUtil::create_crc64( const char *rawData, S_ULONG_32 len)
{
    return build_crc64((const S_UINT_8 *)rawData, len);
}

S_ULONG_64 CRCUtil::build_crc64nocase( const S_UINT_8 *buffer, int bytes)
{
    S_ULONG_64 crc = 0;
    ...
    for ( const S_UINT_8 *p = buffer; bytes > 0; bytes-- ) 
    {
        //S_UINT_8 ch = tolower(*p++);
		S_UINT_8 ch = ACE_OS::ace_tolower(*p++);
        crc = crc_table64_[(crc ^ ch) & 0xff] ^ (crc >> 8);
    }

    return crc;
}

S_ULONG_64 CRCUtil::create_crc64nocase( const char *string)
{
    S_ULONG_64 crc = 0;
