...
void CommandTestImpl::gts_linkdo()
{
	...
	case AOI_UNITMOVE_NTF:
		{
			Pro_AppUnitMove_ntf* ntf =dynamic_cast<Pro_AppUnitMove_ntf*>(recv);
...
void CommandTestImpl::moveto( int x, int y, int z)
{
	...
	Pro_AppUnitMove_req* req =new Pro_AppUnitMove_req();
	req->unittype =0;
	req->unitid =chrid_;
	req->locationx_ =x_;
	req->locationy_ =y_;
