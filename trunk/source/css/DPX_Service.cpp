...
void DPX_Service::act (const void *pact)
{
	server_link_ =reinterpret_cast<DPXLink*>(const_cast<void*>(pact));
...
void DPX_Service::set_act( const void *pact)
{
	server_link_ =reinterpret_cast<DPXLink*>(const_cast<void*>(pact));
	this->server_index_ =server_link_->get_serverindex();
}
...
{
	return (int)CSSAPP_SERVICE_DPX;
}

void DPX_Service::begin_registservice_2()
{
	//等待回复
...
bool DPX_Service::recv_msg( BasicProtocol* pro)
{
	...
	{
		NETCMD_FUN_MAP fun =boost::bind( &CSSSvr::dpx_netadapter, CSSMODULE, _1, _2);
